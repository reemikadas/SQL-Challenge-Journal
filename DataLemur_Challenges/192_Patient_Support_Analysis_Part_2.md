# DataLemur Challenge 192: Patient Support Analysis (Part 2)

**Source:** [View challenge](https://datalemur.com/questions/uncategorized-calls-percentage)
...
~~~sql
SELECT
  ROUND(100.0 * uncategorised_call / total_call, 1) AS uncategorised_call_pct
FROM
  (SELECT
    COUNT(CASE WHEN call_category IS NULL OR call_category ='n/a'
          THEN case_id END) AS uncategorised_call,
    COUNT(case_id) AS total_call
  FROM callers) AS call_count_records
;
~~~
