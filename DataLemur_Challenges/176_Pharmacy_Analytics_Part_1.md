# DataLemur Challenge 176: Pharmacy Analytics (Part 1)

**Source:** [View challenge](https://datalemur.com/questions/top-profitable-drugs)
...
WITH excluded_list AS (
    ...
    HAVING COUNT(*) > 1
    ORDER BY drug, manufacturer
),
  filtered_table AS (
    SELECT * FROM pharmacy_sales
    WHERE drug NOT IN (SELECT drug FROM excluded_list)
    AND manufacturer NOT IN (SELECT manufacturer FROM excluded_list)
)
SELECT
  drug,
